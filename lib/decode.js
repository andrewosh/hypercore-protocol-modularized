...
function DecoderStream (key, dk, opts) {
  if (!(this instanceof DecoderStream)) return new DecoderStream(key, dk, opts)
  if (!opts) opts = {}
...
DecoderStream.prototype._transform = function (data, enc, cb) {
  ...
  return this._parse(data, 0, cb)
}

DecoderStream.prototype._parse = function (data, start, cb) {
  console.log('IN DECODER PARSE')
  ...
  while (start < data.length && !this.destroyed) {
    ...
    if (!decrypted && this._xor) {
      console.log('CALLING PARSE AGAIN')
...
DecoderStream.prototype._parseLength = function (data, start) {
  while (!this._missing && start < data.length) {
    ...
    if (this._pointer >= this._length.length) return this.destroy(errors.tooBig())
  }
...
DecoderStream.prototype._sameKey = function () {
  console.log('IN SAME KEY, this.discoveryKey:', this.discoveryKey, 'remote:', this.remoteDiscoveryKey)
