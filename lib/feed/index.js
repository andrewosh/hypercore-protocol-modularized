...
FeedStream.prototype.close = function () {
  var feeds = this.feeds
  this.feeds = []
  for (var i = 0; i < feeds.length; i++) feeds[i]._onclose()
}

FeedStream.prototype.maybeFinalize = function () {
  if (!this.expectedFeeds) this.finalize()
}

FeedStream.prototype.finalize = function () {
...
FeedStream.prototype._write = function (msg, enc, cb) {
  ...
  if (!ch) {
    console.log('DESTROYING BECAUSE NO CHANNEL')
...
FeedStream.prototype.feed = function (key, opts) {
  ...
  console.log('ch._buffer:', ch._buffer)
...
FeedStream.prototype._onhandshake = function (handshake) {
  ...
  console.log('in onhandshake, handshake:', handshake)
...
FeedStream.prototype.remoteSupports = function (name) {
